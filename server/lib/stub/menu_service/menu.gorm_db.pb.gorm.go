...
type MenuORM struct {
	...
	MenuID      uint64 `gorm:"primary_key;not null"`
	MenuMenuID  *uint64
	Menus       []*MenuORM   `gorm:"foreignkey:MenuMenuID;association_foreignkey:MenuID;preload:true"`
	Module      *ModuleORM   `gorm:"foreignkey:XModuleID;association_foreignkey:ModuleID"`
	ModuleID    uint64       `gorm:"column:ModuleID"`
	ParentID    uint64       `gorm:"column:ParentID"`
	RootPath    string       `gorm:"type:varchar(255)"`
	Task        *MenuTaskORM `gorm:"foreignkey:XMenuID;association_foreignkey:MenuID"`
	UpdatedAt   *time.Time
	UpdatedByID uint64
...
type MenuDisableORM struct {
	...
	MenuDisableID uint64   `gorm:"primary_key;not null"`
	MenuId        *uint64
...
func DefaultStrictUpdateMenu(ctx context.Context, in *Menu, db *gorm.DB) (*Menu, error) {
	...
	lockedRow := &MenuORM{}
	db.Model(&ormObj).Set("gorm:query_option", "FOR UPDATE").Where("menu_id=?", ormObj.MenuID).First(lockedRow)
	...
	filterMenus.MenuMenuID = new(uint64)
	*filterMenus.MenuMenuID = ormObj.MenuID
	if err = db.Where(filterMenus).Delete(MenuORM{}).Error; err != nil {
		return nil, err
	}
	filterTask := MenuTaskORM{}
	if ormObj.MenuID == 0 {
		return nil, errors.EmptyIdError
	}
	filterTask.XMenuID = new(uint64)
	*filterTask.XMenuID = ormObj.MenuID
	if err = db.Where(filterTask).Delete(MenuTaskORM{}).Error; err != nil {
		return nil, err
	}
	...
	for i, f := range updateMask.Paths {
		...
		if !updatedModule && strings.HasPrefix(f, prefix+"Module.") {
			...
			if patchee.Module == nil {
				patchee.Module = &Module{}
			}
			if o, err := DefaultApplyFieldMaskModule(ctx, patchee.Module, patcher.Module, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"Module.", db); err != nil {
				return nil, err
			} else {
				patchee.Module = o
			}
		...
		if !updatedTask && strings.HasPrefix(f, prefix+"Task.") {
			...
			if patchee.Task == nil {
				patchee.Task = &MenuTask{}
			}
			if o, err := DefaultApplyFieldMaskMenuTask(ctx, patchee.Task, patcher.Task, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"Task.", db); err != nil {
				return nil, err
			} else {
				patchee.Task = o
			}
...
func DefaultApplyFieldMaskMenuLicense(ctx context.Context, patchee *MenuLicense, patcher *MenuLicense, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*MenuLicense, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if !updatedChargeDate && strings.HasPrefix(f, prefix+"ChargeDate.") {
			if patcher.ChargeDate == nil {
				patchee.ChargeDate = nil
				continue
			}
			if patchee.ChargeDate == nil {
				patchee.ChargeDate = &timestamppb.Timestamp{}
			}
			childMask := &field_mask.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
				if trimPath := strings.TrimPrefix(updateMask.Paths[j], prefix+"ChargeDate."); trimPath != updateMask.Paths[j] {
					childMask.Paths = append(childMask.Paths, trimPath)
				}
			}
			if err := gorm1.MergeWithMask(patcher.ChargeDate, patchee.ChargeDate, childMask); err != nil {
				return nil, nil
			}
		}
		if f == prefix+"ChargeDate" {
			updatedChargeDate = true
