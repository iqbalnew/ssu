...
type RoleORM struct {
	...
	UserType        *UserTypeORM        // Tag gorm nya di kosonging
...
func (m *Role) ToORM(ctx context.Context) (RoleORM, error) {
	...
	// Repeated type BeneficiaryAccountID is not an ORMable message type
...
type RoleAuthorityORM struct {
	AuthorityLevel   *AuthorityLevelORM // tag gorm nya di kosongin
...
type ProductAuthorityORM struct {
	AuthorityLevel     *AuthorityLevelORM // Tag gorm nya di kosongin
	AuthorityLevelID   uint64             `gorm:"column:AuthorityLevelID;not null"`
	CreatedAt          *time.Time         `gorm:"not null"`
	CreatedByID        uint64             `gorm:"column:CreatedByID"`
	ProductAuthorityID uint64             `gorm:"column:productAuthorityID;primary_key;not null"`
	ProductID          uint64             `gorm:"column:ProductID"`
	UpdatedAt          *time.Time         `gorm:"not null"`
	UpdatedByID        uint64             `gorm:"column:UpdatedByID"`
}
...
// TableName overrides the default tablename generated by GORM
func (ProductAuthorityORM) TableName() string {
	return "product_authorities"
}

// ToORM runs the BeforeToORM hook if present, converts the fields of this
// object to ORM format, runs the AfterToORM hook, then returns the ORM object
func (m *ProductAuthority) ToORM(ctx context.Context) (ProductAuthorityORM, error) {
	to := ProductAuthorityORM{}
	var err error
	if prehook, ok := interface{}(m).(ProductAuthorityWithBeforeToORM); ok {
		if err = prehook.BeforeToORM(ctx, &to); err != nil {
			return to, err
		}
	}
	to.ProductAuthorityID = m.ProductAuthorityID
	to.ProductID = m.ProductID
	to.AuthorityLevelID = m.AuthorityLevelID
	if m.AuthorityLevel != nil {
		tempAuthorityLevel, err := m.AuthorityLevel.ToORM(ctx)
	...
	if posthook, ok := interface{}(m).(ProductAuthorityWithAfterToORM); ok {
		err = posthook.AfterToORM(ctx, &to)
	}
	return to, err
}
...
// ToPB runs the BeforeToPB hook if present, converts the fields of this
// object to PB format, runs the AfterToPB hook, then returns the PB object
func (m *ProductAuthorityORM) ToPB(ctx context.Context) (ProductAuthority, error) {
	to := ProductAuthority{}
	var err error
	if prehook, ok := interface{}(m).(ProductAuthorityWithBeforeToPB); ok {
		if err = prehook.BeforeToPB(ctx, &to); err != nil {
			return to, err
		}
	}
	...
	if posthook, ok := interface{}(m).(ProductAuthorityWithAfterToPB); ok {
		err = posthook.AfterToPB(ctx, &to)
	}
	return to, err
}
...
// The following are interfaces you can implement for special behavior during ORM/PB conversions
// of type ProductAuthority the arg will be the target, the caller the one being converted from

// ProductAuthorityBeforeToORM called before default ToORM code
type ProductAuthorityWithBeforeToORM interface {
	BeforeToORM(context.Context, *ProductAuthorityORM) error
}

// ProductAuthorityAfterToORM called after default ToORM code
type ProductAuthorityWithAfterToORM interface {
	AfterToORM(context.Context, *ProductAuthorityORM) error
}

// ProductAuthorityBeforeToPB called before default ToPB code
type ProductAuthorityWithBeforeToPB interface {
	BeforeToPB(context.Context, *ProductAuthority) error
}

// ProductAuthorityAfterToPB called after default ToPB code
type ProductAuthorityWithAfterToPB interface {
	AfterToPB(context.Context, *ProductAuthority) error
}
...
func DefaultApplyFieldMaskRoleAuthority(ctx context.Context, patchee *RoleAuthority, patcher *RoleAuthority, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*RoleAuthority, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if !updatedProduct && strings.HasPrefix(f, prefix+"Product.") {
			if patcher.Product == nil {
				patchee.Product = nil
				continue
			}
			if patchee.Product == nil {
				patchee.Product = &Product{}
			}
			childMask := &field_mask.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
				if trimPath := strings.TrimPrefix(updateMask.Paths[j], prefix+"Product."); trimPath != updateMask.Paths[j] {
					childMask.Paths = append(childMask.Paths, trimPath)
				}
			}
			if err := gorm1.MergeWithMask(patcher.Product, patchee.Product, childMask); err != nil {
				return nil, nil
			}
		}
		if f == prefix+"Product" {
...
// DefaultCreateProductAuthority executes a basic gorm create call
func DefaultCreateProductAuthority(ctx context.Context, in *ProductAuthority, db *gorm.DB) (*ProductAuthority, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(ProductAuthorityORMWithBeforeCreate_); ok {
		if db, err = hook.BeforeCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(ProductAuthorityORMWithAfterCreate_); ok {
		if err = hook.AfterCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
...
type ProductAuthorityORMWithBeforeCreate_ interface {
	BeforeCreate_(context.Context, *gorm.DB) (*gorm.DB, error)
}
type ProductAuthorityORMWithAfterCreate_ interface {
	AfterCreate_(context.Context, *gorm.DB) error
}

func DefaultReadProductAuthority(ctx context.Context, in *ProductAuthority, db *gorm.DB) (*ProductAuthority, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if ormObj.ProductAuthorityID == 0 {
		return nil, errors.EmptyIdError
	}
	if hook, ok := interface{}(&ormObj).(ProductAuthorityORMWithBeforeReadApplyQuery); ok {
		if db, err = hook.BeforeReadApplyQuery(ctx, db); err != nil {
			return nil, err
		}
	}
	if db, err = gorm1.ApplyFieldSelection(ctx, db, nil, &ProductAuthorityORM{}); err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(ProductAuthorityORMWithBeforeReadFind); ok {
		if db, err = hook.BeforeReadFind(ctx, db); err != nil {
			return nil, err
		}
	}
	ormResponse := ProductAuthorityORM{}
	if err = db.Where(&ormObj).First(&ormResponse).Error; err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormResponse).(ProductAuthorityORMWithAfterReadFind); ok {
		if err = hook.AfterReadFind(ctx, db); err != nil {
			return nil, err
		}
	}
...
type ProductAuthorityORMWithBeforeReadApplyQuery interface {
	BeforeReadApplyQuery(context.Context, *gorm.DB) (*gorm.DB, error)
}
type ProductAuthorityORMWithBeforeReadFind interface {
	BeforeReadFind(context.Context, *gorm.DB) (*gorm.DB, error)
}
type ProductAuthorityORMWithAfterReadFind interface {
	AfterReadFind(context.Context, *gorm.DB) error
}

func DefaultDeleteProductAuthority(ctx context.Context, in *ProductAuthority, db *gorm.DB) error {
	if in == nil {
		return errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(ProductAuthorityORMWithBeforeDelete_); ok {
		if db, err = hook.BeforeDelete_(ctx, db); err != nil {
			return err
		}
	}
	err = db.Where(&ormObj).Delete(&ProductAuthorityORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := interface{}(&ormObj).(ProductAuthorityORMWithAfterDelete_); ok {
		err = hook.AfterDelete_(ctx, db)
	}
	return err
}

type ProductAuthorityORMWithBeforeDelete_ interface {
	BeforeDelete_(context.Context, *gorm.DB) (*gorm.DB, error)
}
type ProductAuthorityORMWithAfterDelete_ interface {
	AfterDelete_(context.Context, *gorm.DB) error
}

func DefaultDeleteProductAuthoritySet(ctx context.Context, in []*ProductAuthority, db *gorm.DB) error {
	if in == nil {
		return errors.NilArgumentError
	}
	var err error
	...
	for _, obj := range in {
		...
		keys = append(keys, ormObj.ProductAuthorityID)
	}
	if hook, ok := (interface{}(&ProductAuthorityORM{})).(ProductAuthorityORMWithBeforeDeleteSet); ok {
		if db, err = hook.BeforeDeleteSet(ctx, in, db); err != nil {
			return err
		}
	}
	err = db.Where("product_authority_id in (?)", keys).Delete(&ProductAuthorityORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := (interface{}(&ProductAuthorityORM{})).(ProductAuthorityORMWithAfterDeleteSet); ok {
		err = hook.AfterDeleteSet(ctx, in, db)
	}
	return err
}

type ProductAuthorityORMWithBeforeDeleteSet interface {
	BeforeDeleteSet(context.Context, []*ProductAuthority, *gorm.DB) (*gorm.DB, error)
}
type ProductAuthorityORMWithAfterDeleteSet interface {
	AfterDeleteSet(context.Context, []*ProductAuthority, *gorm.DB) error
}

// DefaultStrictUpdateProductAuthority clears / replaces / appends first level 1:many children and then executes a gorm update call
func DefaultStrictUpdateProductAuthority(ctx context.Context, in *ProductAuthority, db *gorm.DB) (*ProductAuthority, error) {
	if in == nil {
		return nil, fmt.Errorf("Nil argument to DefaultStrictUpdateProductAuthority")
	}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	}
	lockedRow := &ProductAuthorityORM{}
	db.Model(&ormObj).Set("gorm:query_option", "FOR UPDATE").Where("productAuthorityID=?", ormObj.ProductAuthorityID).First(lockedRow)
	if hook, ok := interface{}(&ormObj).(ProductAuthorityORMWithBeforeStrictUpdateCleanup); ok {
		if db, err = hook.BeforeStrictUpdateCleanup(ctx, db); err != nil {
			return nil, err
		}
	}
	if hook, ok := interface{}(&ormObj).(ProductAuthorityORMWithBeforeStrictUpdateSave); ok {
		if db, err = hook.BeforeStrictUpdateSave(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(ProductAuthorityORMWithAfterStrictUpdateSave); ok {
		if err = hook.AfterStrictUpdateSave(ctx, db); err != nil {
			return nil, err
		}
	}
...
type ProductAuthorityORMWithBeforeStrictUpdateCleanup interface {
	BeforeStrictUpdateCleanup(context.Context, *gorm.DB) (*gorm.DB, error)
}
type ProductAuthorityORMWithBeforeStrictUpdateSave interface {
	BeforeStrictUpdateSave(context.Context, *gorm.DB) (*gorm.DB, error)
}
type ProductAuthorityORMWithAfterStrictUpdateSave interface {
	AfterStrictUpdateSave(context.Context, *gorm.DB) error
}

// DefaultPatchProductAuthority executes a basic gorm update call with patch behavior
func DefaultPatchProductAuthority(ctx context.Context, in *ProductAuthority, updateMask *field_mask.FieldMask, db *gorm.DB) (*ProductAuthority, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	var pbObj ProductAuthority
	var err error
	if hook, ok := interface{}(&pbObj).(ProductAuthorityWithBeforePatchRead); ok {
		if db, err = hook.BeforePatchRead(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	if hook, ok := interface{}(&pbObj).(ProductAuthorityWithBeforePatchApplyFieldMask); ok {
		if db, err = hook.BeforePatchApplyFieldMask(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	if _, err := DefaultApplyFieldMaskProductAuthority(ctx, &pbObj, in, updateMask, "", db); err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&pbObj).(ProductAuthorityWithBeforePatchSave); ok {
		if db, err = hook.BeforePatchSave(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	pbResponse, err := DefaultStrictUpdateProductAuthority(ctx, &pbObj, db)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(pbResponse).(ProductAuthorityWithAfterPatchSave); ok {
		if err = hook.AfterPatchSave(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
...
type ProductAuthorityWithBeforePatchRead interface {
	BeforePatchRead(context.Context, *ProductAuthority, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type ProductAuthorityWithBeforePatchApplyFieldMask interface {
	BeforePatchApplyFieldMask(context.Context, *ProductAuthority, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type ProductAuthorityWithBeforePatchSave interface {
	BeforePatchSave(context.Context, *ProductAuthority, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type ProductAuthorityWithAfterPatchSave interface {
	AfterPatchSave(context.Context, *ProductAuthority, *field_mask.FieldMask, *gorm.DB) error
}

// DefaultPatchSetProductAuthority executes a bulk gorm update call with patch behavior
func DefaultPatchSetProductAuthority(ctx context.Context, objects []*ProductAuthority, updateMasks []*field_mask.FieldMask, db *gorm.DB) ([]*ProductAuthority, error) {
	if len(objects) != len(updateMasks) {
		return nil, fmt.Errorf(errors.BadRepeatedFieldMaskTpl, len(updateMasks), len(objects))
	}

	results := make([]*ProductAuthority, 0, len(objects))
	for i, patcher := range objects {
		pbResponse, err := DefaultPatchProductAuthority(ctx, patcher, updateMasks[i], db)
		if err != nil {
			return nil, err
		}
...
// DefaultApplyFieldMaskProductAuthority patches an pbObject with patcher according to a field mask.
func DefaultApplyFieldMaskProductAuthority(ctx context.Context, patchee *ProductAuthority, patcher *ProductAuthority, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*ProductAuthority, error) {
	if patcher == nil {
		return nil, nil
	} else if patchee == nil {
		return nil, errors.NilArgumentError
	...
	var updatedUpdatedAt bool
	for i, f := range updateMask.Paths {
		if f == prefix+"ProductAuthorityID" {
			patchee.ProductAuthorityID = patcher.ProductAuthorityID
			continue
		}
		if f == prefix+"ProductID" {
			patchee.ProductID = patcher.ProductID
...
// DefaultListProductAuthority executes a gorm list call
func DefaultListProductAuthority(ctx context.Context, db *gorm.DB) ([]*ProductAuthority, error) {
	in := ProductAuthority{}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(ProductAuthorityORMWithBeforeListApplyQuery); ok {
		if db, err = hook.BeforeListApplyQuery(ctx, db); err != nil {
			return nil, err
		}
	}
	db, err = gorm1.ApplyCollectionOperators(ctx, db, &ProductAuthorityORM{}, &ProductAuthority{}, nil, nil, nil, nil)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(ProductAuthorityORMWithBeforeListFind); ok {
		if db, err = hook.BeforeListFind(ctx, db); err != nil {
			return nil, err
		}
	}
	db = db.Where(&ormObj)
	db = db.Order("productAuthorityID")
	ormResponse := []ProductAuthorityORM{}
	if err := db.Find(&ormResponse).Error; err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(ProductAuthorityORMWithAfterListFind); ok {
		if err = hook.AfterListFind(ctx, db, &ormResponse); err != nil {
			return nil, err
		}
	}
	pbResponse := []*ProductAuthority{}
	for _, responseEntry := range ormResponse {
		temp, err := responseEntry.ToPB(ctx)
		if err != nil {
			return nil, err
...
type ProductAuthorityORMWithBeforeListApplyQuery interface {
	BeforeListApplyQuery(context.Context, *gorm.DB) (*gorm.DB, error)
}
type ProductAuthorityORMWithBeforeListFind interface {
	BeforeListFind(context.Context, *gorm.DB) (*gorm.DB, error)
}
type ProductAuthorityORMWithAfterListFind interface {
	AfterListFind(context.Context, *gorm.DB, *[]ProductAuthorityORM) error
}
