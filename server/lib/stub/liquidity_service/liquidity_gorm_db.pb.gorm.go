...
type LiquiditySchemesORM struct {
	Cashflow       []*LiquidityCashflowORM `gorm:"foreignkey:LiquiditySchemesId;association_foreignkey:Id"`
...
type LiquidityCashflowORM struct {
	Beneficiaries      []*LiquidityBeneficiariesORM `gorm:"foreignkey:LiquidityCashflowId;association_foreignkey:Id"`
	CreatedAt          *time.Time
	DeletedAt          *time.Time
	Id                 uint64 `gorm:"primary_key;not null"`
	LiquiditySchemesId *uint64
	Period             string
	PeriodTimes        string `gorm:"type:jsonb"`
	Recurring          bool
	RecurringDate      uint64
	RecurringDays      uint64
	RecurringValue     string
	Sources            []*LiquiditySourcesORM `gorm:"foreignkey:LiquidityCashflowId;association_foreignkey:Id"`
	TBABottomValue     float32
	TBARoundedBase     float32
	TBATopValue        float32
	TBAType            string
	TBAValue           float32
	Type               string
...
func (m *LiquidityCashflow) ToORM(ctx context.Context) (LiquidityCashflowORM, error) {
	...
	to.RecurringDays = m.RecurringDays
	if m.CreatedAt != nil {
		t := m.CreatedAt.AsTime()
		to.CreatedAt = &t
	}
...
type LiquiditySchedulesORM struct {
	Cashflow     *LiquidityCashflowORM `gorm:"foreignkey:CashflowId;association_foreignkey:Id"`
	CashflowID   uint64
	CashflowId   *uint64
	CreatedAt    *time.Time
	DeletedAt    *time.Time
...
func (m *LiquiditySchedules) ToORM(ctx context.Context) (LiquiditySchedulesORM, error) {
	...
	if m.Cashflow != nil {
		tempCashflow, err := m.Cashflow.ToORM(ctx)
		if err != nil {
			return to, err
		}
		to.Cashflow = &tempCashflow
	}
...
func (m *LiquiditySchedulesORM) ToPB(ctx context.Context) (LiquiditySchedules, error) {
	...
	to.CashflowID = m.CashflowID
	if m.Cashflow != nil {
		tempCashflow, err := m.Cashflow.ToPB(ctx)
		if err != nil {
			return to, err
		}
		to.Cashflow = &tempCashflow
...
type HistoriesORM struct {
	CreatedAt        *time.Time
	Data             string `gorm:"type:jsonb"`
	FeeData          string `gorm:"type:jsonb"`
	FeeStatus        int32
	FeeStatusMessage string
	Id               uint64 `gorm:"primary_key;not null"`
...
// TableName overrides the default tablename generated by GORM
func (HistoriesORM) TableName() string {
	return "histories"
}

// ToORM runs the BeforeToORM hook if present, converts the fields of this
// object to ORM format, runs the AfterToORM hook, then returns the ORM object
func (m *Histories) ToORM(ctx context.Context) (HistoriesORM, error) {
	to := HistoriesORM{}
	var err error
	if prehook, ok := interface{}(m).(HistoriesWithBeforeToORM); ok {
		if err = prehook.BeforeToORM(ctx, &to); err != nil {
			return to, err
		}
	}
	...
	to.FeeData = m.FeeData
	to.FeeStatus = int32(m.FeeStatus)
	to.JournalSquence = m.JournalSquence
	to.StatusMessage = m.StatusMessage
	to.FeeStatusMessage = m.FeeStatusMessage
	to.Data = m.Data
	if m.CreatedAt != nil {
		t := m.CreatedAt.AsTime()
		to.CreatedAt = &t
	...
	if posthook, ok := interface{}(m).(HistoriesWithAfterToORM); ok {
		err = posthook.AfterToORM(ctx, &to)
	}
	return to, err
}
...
// ToPB runs the BeforeToPB hook if present, converts the fields of this
// object to PB format, runs the AfterToPB hook, then returns the PB object
func (m *HistoriesORM) ToPB(ctx context.Context) (Histories, error) {
	to := Histories{}
	var err error
	if prehook, ok := interface{}(m).(HistoriesWithBeforeToPB); ok {
		if err = prehook.BeforeToPB(ctx, &to); err != nil {
			return to, err
		}
	}
	...
	to.TransactionId = m.TransactionId
	to.TaskId = m.TaskId
	to.Status = Histories_Statuses(m.Status)
	to.FeeData = m.FeeData
	to.FeeStatus = Histories_Statuses(m.FeeStatus)
	to.JournalSquence = m.JournalSquence
	to.StatusMessage = m.StatusMessage
	to.FeeStatusMessage = m.FeeStatusMessage
	to.Data = m.Data
	...
	if posthook, ok := interface{}(m).(HistoriesWithAfterToPB); ok {
		err = posthook.AfterToPB(ctx, &to)
	}
	return to, err
}
...
// The following are interfaces you can implement for special behavior during ORM/PB conversions
// of type Histories the arg will be the target, the caller the one being converted from

// HistoriesBeforeToORM called before default ToORM code
type HistoriesWithBeforeToORM interface {
	BeforeToORM(context.Context, *HistoriesORM) error
}

// HistoriesAfterToORM called after default ToORM code
type HistoriesWithAfterToORM interface {
	AfterToORM(context.Context, *HistoriesORM) error
}

// HistoriesBeforeToPB called before default ToPB code
type HistoriesWithBeforeToPB interface {
	BeforeToPB(context.Context, *Histories) error
}

// HistoriesAfterToPB called after default ToPB code
type HistoriesWithAfterToPB interface {
	AfterToPB(context.Context, *Histories) error
}
...
func DefaultStrictUpdateLiquiditySchemes(ctx context.Context, in *LiquiditySchemes, db *gorm.DB) (*LiquiditySchemes, error) {
	...
	filterCashflow.LiquiditySchemesId = new(uint64)
	*filterCashflow.LiquiditySchemesId = ormObj.Id
...
func DefaultStrictUpdateLiquidityCashflow(ctx context.Context, in *LiquidityCashflow, db *gorm.DB) (*LiquidityCashflow, error) {
	...
	filterBeneficiaries.LiquidityCashflowId = new(uint64)
	*filterBeneficiaries.LiquidityCashflowId = ormObj.Id
	if err = db.Where(filterBeneficiaries).Delete(LiquidityBeneficiariesORM{}).Error; err != nil {
		return nil, err
	}
	filterSources := LiquiditySourcesORM{}
	if ormObj.Id == 0 {
		return nil, errors.EmptyIdError
	}
	filterSources.LiquidityCashflowId = new(uint64)
	*filterSources.LiquidityCashflowId = ormObj.Id
	if err = db.Where(filterSources).Delete(LiquiditySourcesORM{}).Error; err != nil {
		return nil, err
	}
...
func DefaultApplyFieldMaskLiquidityCashflow(ctx context.Context, patchee *LiquidityCashflow, patcher *LiquidityCashflow, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*LiquidityCashflow, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"PeriodTimes" {
			patchee.PeriodTimes = patcher.PeriodTimes
			continue
		}
		if f == prefix+"TBAType" {
		...
		if f == prefix+"RecurringDays" {
			patchee.RecurringDays = patcher.RecurringDays
			continue
		}
		if !updatedCreatedAt && strings.HasPrefix(f, prefix+"CreatedAt.") {
			if patcher.CreatedAt == nil {
...
func DefaultApplyFieldMaskLiquiditySchedules(ctx context.Context, patchee *LiquiditySchedules, patcher *LiquiditySchedules, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*LiquiditySchedules, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if !updatedCashflow && strings.HasPrefix(f, prefix+"Cashflow.") {
			updatedCashflow = true
			if patcher.Cashflow == nil {
				patchee.Cashflow = nil
				continue
			}
			if patchee.Cashflow == nil {
				patchee.Cashflow = &LiquidityCashflow{}
			}
			if o, err := DefaultApplyFieldMaskLiquidityCashflow(ctx, patchee.Cashflow, patcher.Cashflow, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"Cashflow.", db); err != nil {
				return nil, err
			} else {
				patchee.Cashflow = o
			}
		...
		if f == prefix+"Executed" {
			patchee.Executed = patcher.Executed
			continue
		}
...
// DefaultCreateHistories executes a basic gorm create call
func DefaultCreateHistories(ctx context.Context, in *Histories, db *gorm.DB) (*Histories, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(HistoriesORMWithBeforeCreate_); ok {
		if db, err = hook.BeforeCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(HistoriesORMWithAfterCreate_); ok {
		if err = hook.AfterCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
...
type HistoriesORMWithBeforeCreate_ interface {
	BeforeCreate_(context.Context, *gorm.DB) (*gorm.DB, error)
}
type HistoriesORMWithAfterCreate_ interface {
	AfterCreate_(context.Context, *gorm.DB) error
}

func DefaultReadHistories(ctx context.Context, in *Histories, db *gorm.DB) (*Histories, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(HistoriesORMWithBeforeReadApplyQuery); ok {
		if db, err = hook.BeforeReadApplyQuery(ctx, db); err != nil {
			return nil, err
		}
	}
	if db, err = gorm1.ApplyFieldSelection(ctx, db, nil, &HistoriesORM{}); err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(HistoriesORMWithBeforeReadFind); ok {
		if db, err = hook.BeforeReadFind(ctx, db); err != nil {
			return nil, err
		}
	}
	ormResponse := HistoriesORM{}
	if err = db.Where(&ormObj).First(&ormResponse).Error; err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormResponse).(HistoriesORMWithAfterReadFind); ok {
		if err = hook.AfterReadFind(ctx, db); err != nil {
			return nil, err
		}
	}
...
type HistoriesORMWithBeforeReadApplyQuery interface {
	BeforeReadApplyQuery(context.Context, *gorm.DB) (*gorm.DB, error)
}
type HistoriesORMWithBeforeReadFind interface {
	BeforeReadFind(context.Context, *gorm.DB) (*gorm.DB, error)
}
type HistoriesORMWithAfterReadFind interface {
	AfterReadFind(context.Context, *gorm.DB) error
}

func DefaultDeleteHistories(ctx context.Context, in *Histories, db *gorm.DB) error {
	if in == nil {
		return errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(HistoriesORMWithBeforeDelete_); ok {
		if db, err = hook.BeforeDelete_(ctx, db); err != nil {
			return err
		}
	}
	err = db.Where(&ormObj).Delete(&HistoriesORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := interface{}(&ormObj).(HistoriesORMWithAfterDelete_); ok {
		err = hook.AfterDelete_(ctx, db)
	}
	return err
}

type HistoriesORMWithBeforeDelete_ interface {
	BeforeDelete_(context.Context, *gorm.DB) (*gorm.DB, error)
}
type HistoriesORMWithAfterDelete_ interface {
	AfterDelete_(context.Context, *gorm.DB) error
}

func DefaultDeleteHistoriesSet(ctx context.Context, in []*Histories, db *gorm.DB) error {
	if in == nil {
		return errors.NilArgumentError
	}
	var err error
	...
	if hook, ok := (interface{}(&HistoriesORM{})).(HistoriesORMWithBeforeDeleteSet); ok {
		if db, err = hook.BeforeDeleteSet(ctx, in, db); err != nil {
			return err
		}
	}
	err = db.Where("id in (?)", keys).Delete(&HistoriesORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := (interface{}(&HistoriesORM{})).(HistoriesORMWithAfterDeleteSet); ok {
		err = hook.AfterDeleteSet(ctx, in, db)
	}
	return err
}

type HistoriesORMWithBeforeDeleteSet interface {
	BeforeDeleteSet(context.Context, []*Histories, *gorm.DB) (*gorm.DB, error)
}
type HistoriesORMWithAfterDeleteSet interface {
	AfterDeleteSet(context.Context, []*Histories, *gorm.DB) error
}

// DefaultStrictUpdateHistories clears / replaces / appends first level 1:many children and then executes a gorm update call
func DefaultStrictUpdateHistories(ctx context.Context, in *Histories, db *gorm.DB) (*Histories, error) {
	if in == nil {
		return nil, fmt.Errorf("Nil argument to DefaultStrictUpdateHistories")
	}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	}
	lockedRow := &HistoriesORM{}
	db.Model(&ormObj).Set("gorm:query_option", "FOR UPDATE").Where("id=?", ormObj.Id).First(lockedRow)
	if hook, ok := interface{}(&ormObj).(HistoriesORMWithBeforeStrictUpdateCleanup); ok {
		if db, err = hook.BeforeStrictUpdateCleanup(ctx, db); err != nil {
			return nil, err
		}
	}
	if hook, ok := interface{}(&ormObj).(HistoriesORMWithBeforeStrictUpdateSave); ok {
		if db, err = hook.BeforeStrictUpdateSave(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(HistoriesORMWithAfterStrictUpdateSave); ok {
		if err = hook.AfterStrictUpdateSave(ctx, db); err != nil {
			return nil, err
		}
	}
...
type HistoriesORMWithBeforeStrictUpdateCleanup interface {
	BeforeStrictUpdateCleanup(context.Context, *gorm.DB) (*gorm.DB, error)
}
type HistoriesORMWithBeforeStrictUpdateSave interface {
	BeforeStrictUpdateSave(context.Context, *gorm.DB) (*gorm.DB, error)
}
type HistoriesORMWithAfterStrictUpdateSave interface {
	AfterStrictUpdateSave(context.Context, *gorm.DB) error
}

// DefaultPatchHistories executes a basic gorm update call with patch behavior
func DefaultPatchHistories(ctx context.Context, in *Histories, updateMask *field_mask.FieldMask, db *gorm.DB) (*Histories, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	var pbObj Histories
	var err error
	if hook, ok := interface{}(&pbObj).(HistoriesWithBeforePatchRead); ok {
		if db, err = hook.BeforePatchRead(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	pbReadRes, err := DefaultReadHistories(ctx, &Histories{Id: in.GetId()}, db)
	if err != nil {
		return nil, err
	}
	pbObj = *pbReadRes
	if hook, ok := interface{}(&pbObj).(HistoriesWithBeforePatchApplyFieldMask); ok {
		if db, err = hook.BeforePatchApplyFieldMask(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	if _, err := DefaultApplyFieldMaskHistories(ctx, &pbObj, in, updateMask, "", db); err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&pbObj).(HistoriesWithBeforePatchSave); ok {
		if db, err = hook.BeforePatchSave(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	pbResponse, err := DefaultStrictUpdateHistories(ctx, &pbObj, db)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(pbResponse).(HistoriesWithAfterPatchSave); ok {
		if err = hook.AfterPatchSave(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
...
type HistoriesWithBeforePatchRead interface {
	BeforePatchRead(context.Context, *Histories, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type HistoriesWithBeforePatchApplyFieldMask interface {
	BeforePatchApplyFieldMask(context.Context, *Histories, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type HistoriesWithBeforePatchSave interface {
	BeforePatchSave(context.Context, *Histories, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type HistoriesWithAfterPatchSave interface {
	AfterPatchSave(context.Context, *Histories, *field_mask.FieldMask, *gorm.DB) error
}

// DefaultPatchSetHistories executes a bulk gorm update call with patch behavior
func DefaultPatchSetHistories(ctx context.Context, objects []*Histories, updateMasks []*field_mask.FieldMask, db *gorm.DB) ([]*Histories, error) {
	if len(objects) != len(updateMasks) {
		return nil, fmt.Errorf(errors.BadRepeatedFieldMaskTpl, len(updateMasks), len(objects))
	}

	results := make([]*Histories, 0, len(objects))
	for i, patcher := range objects {
		pbResponse, err := DefaultPatchHistories(ctx, patcher, updateMasks[i], db)
		if err != nil {
			return nil, err
		}
...
// DefaultApplyFieldMaskHistories patches an pbObject with patcher according to a field mask.
func DefaultApplyFieldMaskHistories(ctx context.Context, patchee *Histories, patcher *Histories, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*Histories, error) {
	if patcher == nil {
		return nil, nil
	} else if patchee == nil {
		return nil, errors.NilArgumentError
	...
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"FeeData" {
			patchee.FeeData = patcher.FeeData
			continue
		}
		if f == prefix+"FeeStatus" {
			patchee.FeeStatus = patcher.FeeStatus
			continue
		}
		...
		if f == prefix+"FeeStatusMessage" {
			patchee.FeeStatusMessage = patcher.FeeStatusMessage
			continue
		}
		if f == prefix+"Data" {
...
// DefaultListHistories executes a gorm list call
func DefaultListHistories(ctx context.Context, db *gorm.DB) ([]*Histories, error) {
	in := Histories{}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(HistoriesORMWithBeforeListApplyQuery); ok {
		if db, err = hook.BeforeListApplyQuery(ctx, db); err != nil {
			return nil, err
		}
	}
	db, err = gorm1.ApplyCollectionOperators(ctx, db, &HistoriesORM{}, &Histories{}, nil, nil, nil, nil)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(HistoriesORMWithBeforeListFind); ok {
		if db, err = hook.BeforeListFind(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(HistoriesORMWithAfterListFind); ok {
		if err = hook.AfterListFind(ctx, db, &ormResponse); err != nil {
			return nil, err
		}
	}
	pbResponse := []*Histories{}
	for _, responseEntry := range ormResponse {
		temp, err := responseEntry.ToPB(ctx)
		if err != nil {
			return nil, err
...
type HistoriesORMWithBeforeListApplyQuery interface {
	BeforeListApplyQuery(context.Context, *gorm.DB) (*gorm.DB, error)
}
type HistoriesORMWithBeforeListFind interface {
	BeforeListFind(context.Context, *gorm.DB) (*gorm.DB, error)
}
type HistoriesORMWithAfterListFind interface {
	AfterListFind(context.Context, *gorm.DB, *[]HistoriesORM) error
}
