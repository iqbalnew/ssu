...
type ProductORM struct {
	BaseHreff       string     `gorm:"column:BaseHreff"`
	CreatedAt       *time.Time `gorm:"column:CreatedAt"`
	DeletedAt       *time.Time `gorm:"column:DeletedAt"`
	Is3RdParty      bool       `gorm:"column:Is3rdParty;default:false;not null"`
	IsBa            bool       `gorm:"column:IsBa;default:false;not null"`
	IsCa            bool       `gorm:"column:IsCa;default:false;not null"`
	IsCu            bool       `gorm:"column:IsCu;default:false;not null"`
	IsTransactional bool       `gorm:"column:IsTransactional;default:false;not null"`
	Name            string     `gorm:"column:Name;type:varchar(100);unique;not null"`
	ParentID        uint64     `gorm:"column:ParentID"`
	ProductID       uint64     `gorm:"column:ProductID;primary_key;not null;auto_increment"`
	RefByTask       bool       `gorm:"column:RefByTask;default:false;not null"`
...
func (m *Product) ToORM(ctx context.Context) (ProductORM, error) {
	...
	to.Name = m.Name
	to.Is3RdParty = m.Is3RdParty
	to.IsTransactional = m.IsTransactional
	to.IsBa = m.IsBa
	to.IsCa = m.IsCa
	to.IsCu = m.IsCu
	to.RefByTask = m.RefByTask
	to.ParentID = m.ParentID
	to.BaseHreff = m.BaseHreff
	if m.CreatedAt != nil {
...
func DefaultDeleteProductSet(ctx context.Context, in []*Product, db *gorm.DB) error {
	...
	for _, obj := range in {
		...
		keys = append(keys, ormObj.ProductID)
	}
	...
	err = db.Where("product_id in (?)", keys).Delete(&ProductORM{}).Error
	if err != nil {
		return err
	}
...
func DefaultStrictUpdateProduct(ctx context.Context, in *Product, db *gorm.DB) (*Product, error) {
	...
	db.Model(&ormObj).Set("gorm:query_option", "FOR UPDATE").Where("ProductID=?", ormObj.ProductID).First(lockedRow)
...
func DefaultApplyFieldMaskProduct(ctx context.Context, patchee *Product, patcher *Product, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*Product, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"Is3RdParty" {
			patchee.Is3RdParty = patcher.Is3RdParty
			continue
		}
		...
		if f == prefix+"IsBa" {
			patchee.IsBa = patcher.IsBa
			continue
		}
		if f == prefix+"IsCa" {
			patchee.IsCa = patcher.IsCa
			continue
		}
		if f == prefix+"IsCu" {
			patchee.IsCu = patcher.IsCu
			continue
		}
		if f == prefix+"RefByTask" {
			patchee.RefByTask = patcher.RefByTask
			continue
		}
		...
		if f == prefix+"BaseHreff" {
			patchee.BaseHreff = patcher.BaseHreff
			continue
		}
