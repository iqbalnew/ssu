...
type AbonnementRetryORM struct {
	...
	AbonnementID        uint64
	AbonnementId        *uint64
	AbonnementInvoice   *AbonnementInvoiceORM `gorm:"foreignkey:AbonnementInvoiceId;association_foreignkey:Id"`
	AbonnementInvoiceID uint64
	AbonnementInvoiceId *uint64
	CreatedAt           *time.Time
	CreatedByID         uint64
	Id                  uint64 `gorm:"primary_key;not null"`
...
func (m *AbonnementRetry) ToORM(ctx context.Context) (AbonnementRetryORM, error) {
	...
	to.AbonnementInvoiceID = m.AbonnementInvoiceID
	if m.AbonnementInvoice != nil {
		tempAbonnementInvoice, err := m.AbonnementInvoice.ToORM(ctx)
		if err != nil {
			return to, err
		}
		to.AbonnementInvoice = &tempAbonnementInvoice
	}
	if m.RetryDate != nil {
...
func (m *AbonnementRetryORM) ToPB(ctx context.Context) (AbonnementRetry, error) {
	...
	to.AbonnementInvoiceID = m.AbonnementInvoiceID
	if m.AbonnementInvoice != nil {
		tempAbonnementInvoice, err := m.AbonnementInvoice.ToPB(ctx)
		if err != nil {
			return to, err
		}
		to.AbonnementInvoice = &tempAbonnementInvoice
...
func (m *AbonnementInvoice) ToORM(ctx context.Context) (AbonnementInvoiceORM, error) {
	...
	to.EsbResponse = m.EsbResponse
	to.RetryCounter = m.RetryCounter
	to.CreatedByID = m.CreatedByID
	if m.CreatedAt != nil {
		t := m.CreatedAt.AsTime()
		to.CreatedAt = &t
...
func DefaultApplyFieldMaskAbonnementRetry(ctx context.Context, patchee *AbonnementRetry, patcher *AbonnementRetry, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*AbonnementRetry, error) {
	...
	var updatedAbonnementInvoice bool
	...
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"AbonnementInvoiceID" {
			patchee.AbonnementInvoiceID = patcher.AbonnementInvoiceID
			continue
		}
		if !updatedAbonnementInvoice && strings.HasPrefix(f, prefix+"AbonnementInvoice.") {
			updatedAbonnementInvoice = true
			if patcher.AbonnementInvoice == nil {
				patchee.AbonnementInvoice = nil
				continue
			}
			if patchee.AbonnementInvoice == nil {
				patchee.AbonnementInvoice = &AbonnementInvoice{}
			}
			if o, err := DefaultApplyFieldMaskAbonnementInvoice(ctx, patchee.AbonnementInvoice, patcher.AbonnementInvoice, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"AbonnementInvoice.", db); err != nil {
				return nil, err
			} else {
				patchee.AbonnementInvoice = o
			}
			continue
		}
		if f == prefix+"AbonnementInvoice" {
			updatedAbonnementInvoice = true
			patchee.AbonnementInvoice = patcher.AbonnementInvoice
			continue
		}
...
func DefaultApplyFieldMaskAbonnementInvoice(ctx context.Context, patchee *AbonnementInvoice, patcher *AbonnementInvoice, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*AbonnementInvoice, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"EsbJournalSeq" {
			patchee.EsbJournalSeq = patcher.EsbJournalSeq
			continue
		}
		if f == prefix+"EsbResponseCode" {
			patchee.EsbResponseCode = patcher.EsbResponseCode
			continue
		}
		if f == prefix+"EsbResponseMessage" {
			patchee.EsbResponseMessage = patcher.EsbResponseMessage
			continue
		}
		if f == prefix+"EsbResponse" {
			patchee.EsbResponse = patcher.EsbResponse
			continue
		}
		if f == prefix+"RetryCounter" {
			patchee.RetryCounter = patcher.RetryCounter
			continue
		}
		if f == prefix+"CreatedByID" {
			patchee.CreatedByID = patcher.CreatedByID
