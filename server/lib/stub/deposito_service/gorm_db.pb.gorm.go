...
type DepositoORM struct {
	AccNoInterest  string `gorm:"type:varchar(100)"`
	AccNoPrincipal string `gorm:"type:varchar(100)"`
	AccNo_Deposito string `gorm:"type:varchar(100)"`
	AccNo_TD       string `gorm:"type:varchar(100)"`
	...
	WhitdrawAt     *time.Time `gorm:"type:varchar(100)"`
}
...
func (m *Deposito) ToORM(ctx context.Context) (DepositoORM, error) {
	...
	to.AccNo_TD = m.AccNo_TD
	to.AccNoPrincipal = m.AccNoPrincipal
	to.AccNoInterest = m.AccNoInterest
...
func DefaultApplyFieldMaskDeposito(ctx context.Context, patchee *Deposito, patcher *Deposito, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*Deposito, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"AccNo_TD" {
			patchee.AccNo_TD = patcher.AccNo_TD
			continue
		}
		if f == prefix+"AccNoPrincipal" {
			patchee.AccNoPrincipal = patcher.AccNoPrincipal
			continue
		}
		if f == prefix+"AccNoInterest" {
			patchee.AccNoInterest = patcher.AccNoInterest
			continue
		}
