...
type RemittanceTransactionORM struct {
	...
	CREDIT_ACCOUNT_HOLDER_NAME    string `gorm:"type:varchar(100)"`
	...
	DEBIT_ACCOUNT_HOLDER_NAME     string `gorm:"type:varchar(100)"`
	DEBIT_ACCOUNT_MAIN            string `gorm:"type:varchar(100)"`
	DEBIT_AMOUNT                  string `gorm:"type:varchar(100)"`
	DEBIT_BANK_NAME               string `gorm:"type:varchar(100)"`
	...
	REMARK_TO_BENEFICIARY         string `gorm:"type:varchar(100)"`
	REMARK_TO_BENEFICIARY_BANK    string `gorm:"type:varchar(100)"`
...
func (m *RemittanceTransaction) ToORM(ctx context.Context) (RemittanceTransactionORM, error) {
	...
	to.DEBIT_ACCOUNT_HOLDER_NAME = m.DEBIT_ACCOUNT_HOLDER_NAME
	to.DEBIT_BANK_NAME = m.DEBIT_BANK_NAME
	to.CREDIT_ACCOUNT_HOLDER_NAME = m.CREDIT_ACCOUNT_HOLDER_NAME
	...
	to.REMARK_TO_BENEFICIARY = m.REMARK_TO_BENEFICIARY
	to.REMARK_TO_BENEFICIARY_BANK = m.REMARK_TO_BENEFICIARY_BANK
...
type RemittanceTemplateORM struct {
	CompanyOwner  string `gorm:"type:varchar(100)"`
	CreatedAt     *time.Time
	CreatedByID   uint64
	CreatedByName string `gorm:"type:varchar(100)"`
	Data          string `gorm:"type:jsonb"`
	TemplateID    uint64 `gorm:"primary_key;not null"`
	TemplateName  string `gorm:"type:varchar(100)"`
	UpdatedAt     *time.Time
}

// TableName overrides the default tablename generated by GORM
func (RemittanceTemplateORM) TableName() string {
	return "remittance_templates"
}

// ToORM runs the BeforeToORM hook if present, converts the fields of this
// object to ORM format, runs the AfterToORM hook, then returns the ORM object
func (m *RemittanceTemplate) ToORM(ctx context.Context) (RemittanceTemplateORM, error) {
	to := RemittanceTemplateORM{}
	var err error
	if prehook, ok := interface{}(m).(RemittanceTemplateWithBeforeToORM); ok {
		if err = prehook.BeforeToORM(ctx, &to); err != nil {
			return to, err
		}
	}
	to.TemplateID = m.TemplateID
	to.CompanyOwner = m.CompanyOwner
	...
	if posthook, ok := interface{}(m).(RemittanceTemplateWithAfterToORM); ok {
		err = posthook.AfterToORM(ctx, &to)
	}
	return to, err
}
...
// ToPB runs the BeforeToPB hook if present, converts the fields of this
// object to PB format, runs the AfterToPB hook, then returns the PB object
func (m *RemittanceTemplateORM) ToPB(ctx context.Context) (RemittanceTemplate, error) {
	to := RemittanceTemplate{}
	var err error
	if prehook, ok := interface{}(m).(RemittanceTemplateWithBeforeToPB); ok {
		if err = prehook.BeforeToPB(ctx, &to); err != nil {
			return to, err
		}
	}
	...
	if posthook, ok := interface{}(m).(RemittanceTemplateWithAfterToPB); ok {
		err = posthook.AfterToPB(ctx, &to)
	}
	return to, err
}
...
// The following are interfaces you can implement for special behavior during ORM/PB conversions
// of type RemittanceTemplate the arg will be the target, the caller the one being converted from

// RemittanceTemplateBeforeToORM called before default ToORM code
type RemittanceTemplateWithBeforeToORM interface {
	BeforeToORM(context.Context, *RemittanceTemplateORM) error
}

// RemittanceTemplateAfterToORM called after default ToORM code
type RemittanceTemplateWithAfterToORM interface {
	AfterToORM(context.Context, *RemittanceTemplateORM) error
}

// RemittanceTemplateBeforeToPB called before default ToPB code
type RemittanceTemplateWithBeforeToPB interface {
	BeforeToPB(context.Context, *RemittanceTemplate) error
}

// RemittanceTemplateAfterToPB called after default ToPB code
type RemittanceTemplateWithAfterToPB interface {
	AfterToPB(context.Context, *RemittanceTemplate) error
}
...
func DefaultApplyFieldMaskRemittanceTransaction(ctx context.Context, patchee *RemittanceTransaction, patcher *RemittanceTransaction, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*RemittanceTransaction, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"DEBIT_ACCOUNT_HOLDER_NAME" {
			patchee.DEBIT_ACCOUNT_HOLDER_NAME = patcher.DEBIT_ACCOUNT_HOLDER_NAME
			continue
		}
		if f == prefix+"DEBIT_BANK_NAME" {
			patchee.DEBIT_BANK_NAME = patcher.DEBIT_BANK_NAME
			continue
		}
		if f == prefix+"CREDIT_ACCOUNT_HOLDER_NAME" {
			patchee.CREDIT_ACCOUNT_HOLDER_NAME = patcher.CREDIT_ACCOUNT_HOLDER_NAME
			continue
		}
		...
		if f == prefix+"REMARK_TO_BENEFICIARY" {
			patchee.REMARK_TO_BENEFICIARY = patcher.REMARK_TO_BENEFICIARY
			continue
		}
		if f == prefix+"REMARK_TO_BENEFICIARY_BANK" {
			patchee.REMARK_TO_BENEFICIARY_BANK = patcher.REMARK_TO_BENEFICIARY_BANK
			continue
		}
	}
	if err != nil {
...
// DefaultCreateRemittanceTemplate executes a basic gorm create call
func DefaultCreateRemittanceTemplate(ctx context.Context, in *RemittanceTemplate, db *gorm.DB) (*RemittanceTemplate, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(RemittanceTemplateORMWithBeforeCreate_); ok {
		if db, err = hook.BeforeCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(RemittanceTemplateORMWithAfterCreate_); ok {
		if err = hook.AfterCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
...
type RemittanceTemplateORMWithBeforeCreate_ interface {
	BeforeCreate_(context.Context, *gorm.DB) (*gorm.DB, error)
}
type RemittanceTemplateORMWithAfterCreate_ interface {
	AfterCreate_(context.Context, *gorm.DB) error
}

func DefaultReadRemittanceTemplate(ctx context.Context, in *RemittanceTemplate, db *gorm.DB) (*RemittanceTemplate, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if ormObj.TemplateID == 0 {
		return nil, errors.EmptyIdError
	}
	if hook, ok := interface{}(&ormObj).(RemittanceTemplateORMWithBeforeReadApplyQuery); ok {
		if db, err = hook.BeforeReadApplyQuery(ctx, db); err != nil {
			return nil, err
		}
	}
	if db, err = gorm1.ApplyFieldSelection(ctx, db, nil, &RemittanceTemplateORM{}); err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(RemittanceTemplateORMWithBeforeReadFind); ok {
		if db, err = hook.BeforeReadFind(ctx, db); err != nil {
			return nil, err
		}
	}
	ormResponse := RemittanceTemplateORM{}
	if err = db.Where(&ormObj).First(&ormResponse).Error; err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormResponse).(RemittanceTemplateORMWithAfterReadFind); ok {
		if err = hook.AfterReadFind(ctx, db); err != nil {
			return nil, err
		}
	}
...
type RemittanceTemplateORMWithBeforeReadApplyQuery interface {
	BeforeReadApplyQuery(context.Context, *gorm.DB) (*gorm.DB, error)
}
type RemittanceTemplateORMWithBeforeReadFind interface {
	BeforeReadFind(context.Context, *gorm.DB) (*gorm.DB, error)
}
type RemittanceTemplateORMWithAfterReadFind interface {
	AfterReadFind(context.Context, *gorm.DB) error
}

func DefaultDeleteRemittanceTemplate(ctx context.Context, in *RemittanceTemplate, db *gorm.DB) error {
	if in == nil {
		return errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(RemittanceTemplateORMWithBeforeDelete_); ok {
		if db, err = hook.BeforeDelete_(ctx, db); err != nil {
			return err
		}
	}
	err = db.Where(&ormObj).Delete(&RemittanceTemplateORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := interface{}(&ormObj).(RemittanceTemplateORMWithAfterDelete_); ok {
		err = hook.AfterDelete_(ctx, db)
	}
	return err
}

type RemittanceTemplateORMWithBeforeDelete_ interface {
	BeforeDelete_(context.Context, *gorm.DB) (*gorm.DB, error)
}
type RemittanceTemplateORMWithAfterDelete_ interface {
	AfterDelete_(context.Context, *gorm.DB) error
}

func DefaultDeleteRemittanceTemplateSet(ctx context.Context, in []*RemittanceTemplate, db *gorm.DB) error {
	if in == nil {
		return errors.NilArgumentError
	}
	var err error
	...
	for _, obj := range in {
		...
		keys = append(keys, ormObj.TemplateID)
	}
	if hook, ok := (interface{}(&RemittanceTemplateORM{})).(RemittanceTemplateORMWithBeforeDeleteSet); ok {
		if db, err = hook.BeforeDeleteSet(ctx, in, db); err != nil {
			return err
		}
	}
	err = db.Where("template_id in (?)", keys).Delete(&RemittanceTemplateORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := (interface{}(&RemittanceTemplateORM{})).(RemittanceTemplateORMWithAfterDeleteSet); ok {
		err = hook.AfterDeleteSet(ctx, in, db)
	}
	return err
}

type RemittanceTemplateORMWithBeforeDeleteSet interface {
	BeforeDeleteSet(context.Context, []*RemittanceTemplate, *gorm.DB) (*gorm.DB, error)
}
type RemittanceTemplateORMWithAfterDeleteSet interface {
	AfterDeleteSet(context.Context, []*RemittanceTemplate, *gorm.DB) error
}

// DefaultStrictUpdateRemittanceTemplate clears / replaces / appends first level 1:many children and then executes a gorm update call
func DefaultStrictUpdateRemittanceTemplate(ctx context.Context, in *RemittanceTemplate, db *gorm.DB) (*RemittanceTemplate, error) {
	if in == nil {
		return nil, fmt.Errorf("Nil argument to DefaultStrictUpdateRemittanceTemplate")
	}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	}
	lockedRow := &RemittanceTemplateORM{}
	db.Model(&ormObj).Set("gorm:query_option", "FOR UPDATE").Where("template_id=?", ormObj.TemplateID).First(lockedRow)
	if hook, ok := interface{}(&ormObj).(RemittanceTemplateORMWithBeforeStrictUpdateCleanup); ok {
		if db, err = hook.BeforeStrictUpdateCleanup(ctx, db); err != nil {
			return nil, err
		}
	}
	if hook, ok := interface{}(&ormObj).(RemittanceTemplateORMWithBeforeStrictUpdateSave); ok {
		if db, err = hook.BeforeStrictUpdateSave(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(RemittanceTemplateORMWithAfterStrictUpdateSave); ok {
		if err = hook.AfterStrictUpdateSave(ctx, db); err != nil {
			return nil, err
		}
	}
...
type RemittanceTemplateORMWithBeforeStrictUpdateCleanup interface {
	BeforeStrictUpdateCleanup(context.Context, *gorm.DB) (*gorm.DB, error)
}
type RemittanceTemplateORMWithBeforeStrictUpdateSave interface {
	BeforeStrictUpdateSave(context.Context, *gorm.DB) (*gorm.DB, error)
}
type RemittanceTemplateORMWithAfterStrictUpdateSave interface {
	AfterStrictUpdateSave(context.Context, *gorm.DB) error
}

// DefaultPatchRemittanceTemplate executes a basic gorm update call with patch behavior
func DefaultPatchRemittanceTemplate(ctx context.Context, in *RemittanceTemplate, updateMask *field_mask.FieldMask, db *gorm.DB) (*RemittanceTemplate, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	var pbObj RemittanceTemplate
	var err error
	if hook, ok := interface{}(&pbObj).(RemittanceTemplateWithBeforePatchRead); ok {
		if db, err = hook.BeforePatchRead(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	if hook, ok := interface{}(&pbObj).(RemittanceTemplateWithBeforePatchApplyFieldMask); ok {
		if db, err = hook.BeforePatchApplyFieldMask(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	if _, err := DefaultApplyFieldMaskRemittanceTemplate(ctx, &pbObj, in, updateMask, "", db); err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&pbObj).(RemittanceTemplateWithBeforePatchSave); ok {
		if db, err = hook.BeforePatchSave(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	pbResponse, err := DefaultStrictUpdateRemittanceTemplate(ctx, &pbObj, db)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(pbResponse).(RemittanceTemplateWithAfterPatchSave); ok {
		if err = hook.AfterPatchSave(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
...
type RemittanceTemplateWithBeforePatchRead interface {
	BeforePatchRead(context.Context, *RemittanceTemplate, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type RemittanceTemplateWithBeforePatchApplyFieldMask interface {
	BeforePatchApplyFieldMask(context.Context, *RemittanceTemplate, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type RemittanceTemplateWithBeforePatchSave interface {
	BeforePatchSave(context.Context, *RemittanceTemplate, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type RemittanceTemplateWithAfterPatchSave interface {
	AfterPatchSave(context.Context, *RemittanceTemplate, *field_mask.FieldMask, *gorm.DB) error
}

// DefaultPatchSetRemittanceTemplate executes a bulk gorm update call with patch behavior
func DefaultPatchSetRemittanceTemplate(ctx context.Context, objects []*RemittanceTemplate, updateMasks []*field_mask.FieldMask, db *gorm.DB) ([]*RemittanceTemplate, error) {
	if len(objects) != len(updateMasks) {
		return nil, fmt.Errorf(errors.BadRepeatedFieldMaskTpl, len(updateMasks), len(objects))
	}

	results := make([]*RemittanceTemplate, 0, len(objects))
	for i, patcher := range objects {
		pbResponse, err := DefaultPatchRemittanceTemplate(ctx, patcher, updateMasks[i], db)
		if err != nil {
			return nil, err
		}
...
// DefaultApplyFieldMaskRemittanceTemplate patches an pbObject with patcher according to a field mask.
func DefaultApplyFieldMaskRemittanceTemplate(ctx context.Context, patchee *RemittanceTemplate, patcher *RemittanceTemplate, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*RemittanceTemplate, error) {
	if patcher == nil {
		return nil, nil
	} else if patchee == nil {
		return nil, errors.NilArgumentError
	...
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"CompanyOwner" {
			patchee.CompanyOwner = patcher.CompanyOwner
			continue
		}
...
// DefaultListRemittanceTemplate executes a gorm list call
func DefaultListRemittanceTemplate(ctx context.Context, db *gorm.DB) ([]*RemittanceTemplate, error) {
	in := RemittanceTemplate{}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(RemittanceTemplateORMWithBeforeListApplyQuery); ok {
		if db, err = hook.BeforeListApplyQuery(ctx, db); err != nil {
			return nil, err
		}
	}
	db, err = gorm1.ApplyCollectionOperators(ctx, db, &RemittanceTemplateORM{}, &RemittanceTemplate{}, nil, nil, nil, nil)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(RemittanceTemplateORMWithBeforeListFind); ok {
		if db, err = hook.BeforeListFind(ctx, db); err != nil {
			return nil, err
		}
	}
	db = db.Where(&ormObj)
	db = db.Order("template_id")
	ormResponse := []RemittanceTemplateORM{}
	if err := db.Find(&ormResponse).Error; err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(RemittanceTemplateORMWithAfterListFind); ok {
		if err = hook.AfterListFind(ctx, db, &ormResponse); err != nil {
			return nil, err
		}
	}
	pbResponse := []*RemittanceTemplate{}
	for _, responseEntry := range ormResponse {
		temp, err := responseEntry.ToPB(ctx)
		if err != nil {
			return nil, err
...
type RemittanceTemplateORMWithBeforeListApplyQuery interface {
	BeforeListApplyQuery(context.Context, *gorm.DB) (*gorm.DB, error)
}
type RemittanceTemplateORMWithBeforeListFind interface {
	BeforeListFind(context.Context, *gorm.DB) (*gorm.DB, error)
}
type RemittanceTemplateORMWithAfterListFind interface {
	AfterListFind(context.Context, *gorm.DB, *[]RemittanceTemplateORM) error
}
