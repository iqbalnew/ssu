...
type TaskORM struct {
	AccountTask      *AccountTaskORM      `gorm:"foreignkey:TaskTaskID;association_foreignkey:TaskID"`
	AnnouncementTask *AnnouncementTaskORM `gorm:"foreignkey:TaskTaskID;association_foreignkey:TaskID"`
	CompanyTask      *CompanyTaskORM      `gorm:"foreignkey:TaskTaskID;association_foreignkey:TaskID"`
	CreatedAt        *time.Time
...
func (m *Task) ToORM(ctx context.Context) (TaskORM, error) {
	...
	if m.CompanyTask != nil {
		tempCompanyTask, err := m.CompanyTask.ToORM(ctx)
		if err != nil {
			return to, err
		}
		to.CompanyTask = &tempCompanyTask
	}
	if m.AnnouncementTask != nil {
		tempAnnouncementTask, err := m.AnnouncementTask.ToORM(ctx)
		if err != nil {
			return to, err
		}
		to.AnnouncementTask = &tempAnnouncementTask
	}
	if m.AccountTask != nil {
		tempAccountTask, err := m.AccountTask.ToORM(ctx)
		if err != nil {
			return to, err
		}
		to.AccountTask = &tempAccountTask
	}
	if m.CreatedAt != nil {
		t := m.CreatedAt.AsTime()
		to.CreatedAt = &t
...
func (m *TaskORM) ToPB(ctx context.Context) (Task, error) {
	...
	to.LastRejectedByID = m.LastRejectedByID
	if m.CompanyTask != nil {
		tempCompanyTask, err := m.CompanyTask.ToPB(ctx)
		if err != nil {
			return to, err
		}
		to.CompanyTask = &tempCompanyTask
	...
	if m.AnnouncementTask != nil {
		tempAnnouncementTask, err := m.AnnouncementTask.ToPB(ctx)
		if err != nil {
			return to, err
		}
		to.AnnouncementTask = &tempAnnouncementTask
	...
	if m.AccountTask != nil {
		tempAccountTask, err := m.AccountTask.ToPB(ctx)
		if err != nil {
			return to, err
		}
		to.AccountTask = &tempAccountTask
...
func DefaultStrictUpdateTask(ctx context.Context, in *Task, db *gorm.DB) (*Task, error) {
	...
	filterAccountTask := AccountTaskORM{}
	if ormObj.TaskID == 0 {
		return nil, errors.EmptyIdError
	}
	filterAccountTask.TaskTaskID = new(uint64)
	*filterAccountTask.TaskTaskID = ormObj.TaskID
	if err = db.Where(filterAccountTask).Delete(AccountTaskORM{}).Error; err != nil {
		return nil, err
	}
	filterAnnouncementTask := AnnouncementTaskORM{}
	if ormObj.TaskID == 0 {
		return nil, errors.EmptyIdError
	}
	filterAnnouncementTask.TaskTaskID = new(uint64)
	*filterAnnouncementTask.TaskTaskID = ormObj.TaskID
	if err = db.Where(filterAnnouncementTask).Delete(AnnouncementTaskORM{}).Error; err != nil {
		return nil, err
	}
	filterCompanyTask := CompanyTaskORM{}
	if ormObj.TaskID == 0 {
		return nil, errors.EmptyIdError
	}
	filterCompanyTask.TaskTaskID = new(uint64)
	*filterCompanyTask.TaskTaskID = ormObj.TaskID
	if err = db.Where(filterCompanyTask).Delete(CompanyTaskORM{}).Error; err != nil {
		return nil, err
	}
...
func DefaultApplyFieldMaskTask(ctx context.Context, patchee *Task, patcher *Task, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*Task, error) {
	...
	var err error
	var updatedCompanyTask bool
	var updatedAnnouncementTask bool
	var updatedAccountTask bool
	var updatedCreatedAt bool
	var updatedUpdatedAt bool
	for i, f := range updateMask.Paths {
		if f == prefix+"TaskID" {
		...
		if !updatedCompanyTask && strings.HasPrefix(f, prefix+"CompanyTask.") {
			updatedCompanyTask = true
			if patcher.CompanyTask == nil {
				patchee.CompanyTask = nil
				continue
			}
			if patchee.CompanyTask == nil {
				patchee.CompanyTask = &CompanyTask{}
			}
			if o, err := DefaultApplyFieldMaskCompanyTask(ctx, patchee.CompanyTask, patcher.CompanyTask, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"CompanyTask.", db); err != nil {
				return nil, err
			} else {
				patchee.CompanyTask = o
			}
			continue
		}
		if f == prefix+"CompanyTask" {
			updatedCompanyTask = true
			patchee.CompanyTask = patcher.CompanyTask
			continue
		}
		if !updatedAnnouncementTask && strings.HasPrefix(f, prefix+"AnnouncementTask.") {
			updatedAnnouncementTask = true
			if patcher.AnnouncementTask == nil {
				patchee.AnnouncementTask = nil
				continue
			}
			if patchee.AnnouncementTask == nil {
				patchee.AnnouncementTask = &AnnouncementTask{}
			}
			if o, err := DefaultApplyFieldMaskAnnouncementTask(ctx, patchee.AnnouncementTask, patcher.AnnouncementTask, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"AnnouncementTask.", db); err != nil {
				return nil, err
			} else {
				patchee.AnnouncementTask = o
			}
			continue
		}
		if f == prefix+"AnnouncementTask" {
			updatedAnnouncementTask = true
			patchee.AnnouncementTask = patcher.AnnouncementTask
			continue
		}
		if !updatedAccountTask && strings.HasPrefix(f, prefix+"AccountTask.") {
			updatedAccountTask = true
			if patcher.AccountTask == nil {
				patchee.AccountTask = nil
				continue
			}
			if patchee.AccountTask == nil {
				patchee.AccountTask = &AccountTask{}
			}
			if o, err := DefaultApplyFieldMaskAccountTask(ctx, patchee.AccountTask, patcher.AccountTask, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"AccountTask.", db); err != nil {
				return nil, err
			} else {
				patchee.AccountTask = o
			}
			continue
		}
		if f == prefix+"AccountTask" {
			updatedAccountTask = true
			patchee.AccountTask = patcher.AccountTask
			continue
		}
		if !updatedCreatedAt && strings.HasPrefix(f, prefix+"CreatedAt.") {
			if patcher.CreatedAt == nil {
