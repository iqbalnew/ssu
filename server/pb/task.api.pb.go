// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.27.1
// 	protoc        v3.6.1
// source: task.api.proto

package pb

import (
	_ "github.com/mwitkow/go-proto-validators"
	_ "google.golang.org/genproto/googleapis/api/annotations"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

var File_task_api_proto protoreflect.FileDescriptor

var file_task_api_proto_rawDesc = []byte{
	,
}

var file_task_api_proto_goTypes = []interface{}{
	(*Empty)(nil),               // 0: task.service.v1.Empty
	(*SaveTaskRequest)(nil),     // 1: task.service.v1.SaveTaskRequest
	(*SetTaskRequest)(nil),      // 2: task.service.v1.SetTaskRequest
	(*ListRequest)(nil),         // 3: task.service.v1.ListRequest
	(*HealthCheckResponse)(nil), // 4: task.service.v1.HealthCheckResponse
	(*SaveTaskResponse)(nil),    // 5: task.service.v1.SaveTaskResponse
	(*SetTaskResponse)(nil),     // 6: task.service.v1.SetTaskResponse
	(*ListTaskResponse)(nil),    // 7: task.service.v1.ListTaskResponse
}
var file_task_api_proto_depIdxs = []int32{
	0, // 0: task.service.v1.TaskService.HealthCheck:input_type -> task.service.v1.Empty
	1, // 1: task.service.v1.TaskService.SaveTaskWithData:input_type -> task.service.v1.SaveTaskRequest
	2, // 2: task.service.v1.TaskService.SetTask:input_type -> task.service.v1.SetTaskRequest
	3, // 3: task.service.v1.TaskService.GetTask:input_type -> task.service.v1.ListRequest
	3, // 4: task.service.v1.TaskService.GetTaskGraph:input_type -> task.service.v1.ListRequest
	4, // 5: task.service.v1.TaskService.HealthCheck:output_type -> task.service.v1.HealthCheckResponse
	5, // 6: task.service.v1.TaskService.SaveTaskWithData:output_type -> task.service.v1.SaveTaskResponse
	6, // 7: task.service.v1.TaskService.SetTask:output_type -> task.service.v1.SetTaskResponse
	7, // 8: task.service.v1.TaskService.GetTask:output_type -> task.service.v1.ListTaskResponse
	7, // 9: task.service.v1.TaskService.GetTaskGraph:output_type -> task.service.v1.ListTaskResponse
	5, // [5:10] is the sub-list for method output_type
	0, // [0:5] is the sub-list for method input_type
	0, // [0:0] is the sub-list for extension type_name
	0, // [0:0] is the sub-list for extension extendee
	0, // [0:0] is the sub-list for field type_name
}

func init() { file_task_api_proto_init() }
func file_task_api_proto_init() {
	if File_task_api_proto != nil {
		return
	}
	file_task_payload_proto_init()
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_task_api_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   0,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_task_api_proto_goTypes,
		DependencyIndexes: file_task_api_proto_depIdxs,
	}.Build()
	File_task_api_proto = out.File
	file_task_api_proto_rawDesc = nil
	file_task_api_proto_goTypes = nil
	file_task_api_proto_depIdxs = nil
}
